...
from medico.models import DadosMedico, Especialidades, DatasAbertas, eh_medico
...
def home(request):
    if (request.method == "GET"):
        ...
        return render(request, 'home.html', {'minhas_consultas': minhas_consultas, 'medicos': medicos, 'especialidades': especialidades, 'eh_medico': eh_medico(request.user)})
...
def escolher_horario(request, id_dados_medicos):
    if (request.method == "GET"):
        ...
        return render(request, 'escolher_horario.html', {'medico': medico, 'datas_abertas': datas_abertas, 'eh_medico': eh_medico(request.user)})
...
def minhas_consultas(request):
    ...
    especialidades = Especialidades.objects.all()
    return render(request, 'minhas_consultas.html', {'minhas_consultas': minhas_consultas, 'eh_medico': eh_medico(request.user), 'especialidades': especialidades})
