...
from .models import Especialidades, DadosMedico, eh_medico, DatasAbertas
...
def cadastro_medico(request):
    ...
    if (request.method == "GET"):
        ...
        return render(request, 'cadastro_medico.html', {'especialidades': especialidades, 'eh_medico': eh_medico(request.user)})
...
def abrir_horario(request):
    ...
    if (request.method == "GET"):
        ...
        return render(request, 'abrir_horario.html', {'dados_medicos': dados_medicos, 'datas_abertas': datas_abertas, 'eh_medico': eh_medico(request.user)})
    elif (request.method == "POST"):
...
def add_documento(request, id_consulta):
    ...
    if (not documento):
        messages.add_message(request, constants.ERROR, 'Por favor, preencha o campo documento')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
